...
{     
    /**
     * 顧客リスト、新着情報のデータ取得し、bladeに渡す
     * 
     * @param Request $request
     * @return list(顧客リスト、新着情報)
     */
    ...
    {   
        ...
        // sessionIdがない場合、ログイン画面に遷移
        if($session_id == ""){
            return redirect('/');
        }

        // 顧客リスト
        $app_list = $this->getAppList($session_id);
        $arrString = print_r($app_list,true);
        Log::debug('app_list:' .$arrString);

        // 新着情報
        $list_info = $this->getInfoList($session_id);
        $arrString = print_r($list_info,true);
        Log::debug('list_info:' .$arrString);

        Log::debug('end:' .__FUNCTION__);
        return view('home.home',compact('app_list' ,'list_info'));
    }
     ...
     */
    private function getAppList($session_id){
     ...
     * @return $data(業者ごとの顧客リスト)
     */
    private function getInfoList($session_id){
        Log::debug('log_start:' .__FUNCTION__);
